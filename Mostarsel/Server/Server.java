...
public class Server extends Thread {
    ...
    public void run() {
        while (true) {
            try {
                System.out.printf("Server is listening and waiting for researchers requests on port %d", this.port);
                Socket cs = ss.accept();
                System.out.println("Researcher is connected to the server");
...
class ClientThread extends Thread {
    ...
    private String KEY = "KHA34934783L";
    ...
    public boolean checkExistenceOfUser(String msg, int type) {
        ...
        try {
            ...
            System.out.println("################");
            if (line != null) {
                byte[] bytesInputKey = KEY.getBytes();
                byte[] bytesEncryptedText = line.getBytes();
                System.out.println(bytesEncryptedText);
                byte[] key = new byte[32];
                byte[] encryptedText = new byte[128];
                int i;
                for (i = 0; i < 32; i++) {
                    if (i < bytesInputKey.length)
                        key[i] = bytesInputKey[i];
                    else
                        key[i] = (byte) 0;
                }

                for (i = 0; i < 128; i++) {
                    if (i < bytesEncryptedText.length)
                        encryptedText[i] = bytesEncryptedText[i];
                    else
                        encryptedText[i] = (byte) 0;
                }

                Object twoFishKey = Twofish.makeKey(key);
                cpt = Twofish.blockDecrypt(encryptedText, 0, twoFishKey);
                ot = new String(cpt);
            }
            ...
            while (!exst && line != null) {
                ...
                if (user.toString().trim().equals(msg.toString().trim())) {
                    exst = true;
                } else {
                    line = br.readLine();
    ...
    public synchronized void addUser(String str) {
        try {
            ...
            byte[] bytesInputKey = KEY.getBytes();
            byte[] bytesPlainText = str.getBytes();
            byte[] key = new byte[32];
            ...
            for (i = 0; i < 128; i++) {
                if (i < bytesPlainText.length)
                    plainText[i] = bytesPlainText[i];
                else
                    plainText[i] = (byte) 0;
            }
            Object twoFishKey = Twofish.makeKey(key);
    ...
    public void sendAllMessage(ChatMessage msg) throws IOException {
        ...
        File nwfile = new File("ChatHistory/" + fileName);
        if (nwfile.exists()) {
            ...
            try {
                ...
                while (line != null) {
                    ...
                    Object twoFishKey = Twofish.makeKey(key);
                    byte[] decrypted = Twofish.blockDecrypt(encryptedText, 0, twoFishKey);
                    String ot = new String(decrypted);
                    msgg.append(ot);
                    msgg.append("\n");
                    line = br.readLine();
                }
     ...
     * File filetosend = new File("PendingFile/" + msg.getDest() + msg.getSource() +
     * msg.getMessage());
    ...
    public synchronized void sendFile(ChatMessage msg) {
        ...
        if (!sent) {
            ...
            /*
             * nwfile = new
             * File("PendingFile/"+msg.getDest()+msg.getSource()+msg.getMessage());
             * 
