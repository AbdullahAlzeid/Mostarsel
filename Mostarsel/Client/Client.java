...
public class Client extends Thread {
    ...
    private final int login = 1, signup = 0, msgtype = 2, file = 3, activeFriendList = 4, activeAnonymousList = 5,
            inactiveFriends = 6;
    ...
    public void run() {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "Error Connecting....Try again.", "Connection Failed!",
                    JOptionPane.ERROR);
            return;
        }
        while (true) {
                ...
                switch (chmsg.getType()) {
                    ...
                    case signup:
                        if (chmsg.getMessage().equals("success")) {
                            ...
                            String success = (chmsg.getType() == login ? "Log In Successful!"
                                    : "Sign Up and Log In Successful!");
                            JOptionPane.showMessageDialog(gui2, success);
                        } else {
                            String failed = (chmsg.getType() == login ? "Wrong Username or Password.Try Again."
                                    : "User Already Exists.");
                    ...
                    case rejected:
                        ...
                        break;
                    case file:
                        boolean validFileExtension = isValidFile(chmsg);
                        ...
                        if (validFileExtension) {
                            ...
                            sockOutput.writeObject(
                                    new ChatMessage(fileReached, chmsg.getMessage(), chmsg.getDest(),
                                            chmsg.getSource()));
                    ...
                    case logout:
                        ...
                        gui2.setVisible(false);
                        gui1.setVisible(true);
                        JOptionPane.showMessageDialog(gui1, "You are Logged Out.", "Log Out",
                                JOptionPane.INFORMATION_MESSAGE);
                        break;
                    case fileReached:
                        System.out.println("acc rec");
                        JOptionPane.showMessageDialog(gui2, "File delivered to " + chmsg.getSource(), "File Transfer",
                                JOptionPane.INFORMATION_MESSAGE);
                        break;
